      ...
      <Grid  container spacing={2}>
        ...
        <Grid xs={9}>
          <GridItem sx={{padding:0}}>
            <TextField sx={{width:'100%'}} size='small' id="standard-basic" label="Standard" variant="standard" />
          </GridItem>
        </Grid>
      </Grid>
    </Box>
...
function SuveyQuestionStack({modalHandler} : SuveyQuestionStackInterface) {
    ...
    const renderQuestions = () => {
      ...
      return (
        Array.from({length : surveyEditInfo.surveyM.qstnM.length}, (_, index) => {
          return <QuestionCard key={surveyEditInfo.surveyM.qstnM[index].id} id={surveyEditInfo.surveyM.qstnM[index].id}
          title={surveyEditInfo.surveyM.qstnM[index].qstnTitle} content={surveyEditInfo.surveyM.qstnM[index].qstnValue} placeholder={''} modalHandler={modalHandler}/>
        })
      )
    };
            ...
            <Stack spacing={2}>
                <Box>
                  ...
                  <Box sx={{height:30, margin:3}}>
                    <Button variant="contained" onClick={handleAddQuestion}>새 문항 추가</Button>
                  </Box>
                </Box>
            </Stack>
      </Box>
...
export default function DocumentEditor() {
   ...
   */
  const [propQuestionEditor, setPropQuestionEditor] = useState<ModalState>({open:false, question:surveyEditInfo.surveyM.qstnM[0]})
  ...
  const modalHandler : ModalHandler= {
    open : (questionId : string) => {
      const questionM = surveyEditInfo.surveyM.qstnM.find((question) =>  question.id == questionId);
      if(questionM) {
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Stack spacing={2}>
        <Item>
          <TextField
            ...
            defaultValue={surveyEditInfo.surveyM ? surveyEditInfo.surveyM.svyTitle : ""}
            />
        </Item>
        <Item>
            <SuveyPropertiesGrid />
      ...
      <QuestionEditor modalState={propQuestionEditor} setModalState={setPropQuestionEditor} handler={modalHandler}/>
    </Box>
  );
}
